...
public class Pathfinding {	
	...
	private static Object[][] getTiles(Level map, Tile current) {
		ArrayList<Object[]> toRet = new ArrayList<Object[]>(8);
		int x, y;
		for (Object[] move : moves) {
			x = current.getLocation().x + (Integer) move[0];
			y = current.getLocation().y + (Integer) move[1];
			try {
	...
	public static Point[] ASTAR(Level map, Tile start, Tile end) {
		...
		PriorityQueue<Tile> openHeap = new PriorityQueue<Tile>();
		Tile current;
